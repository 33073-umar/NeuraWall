...
# Step 4: Analyze traffic and identify malicious IPs and packets
def analyze_traffic_with_models(csv_file):
    ...
    for col in required_columns:
        if col not in data.columns:
            raise ValueError(f"The required column '{col}' is missing in the input data.")
    ...
    if not malicious_ips.empty:
        ...
        # Save malicious IPs to a CSV file
        malicious_ip_file = "logs/malicious_ips.csv"
        with open(malicious_ip_file, "a") as f:
            for ip in malicious_ips.index:
                f.write(f"{ip}\n")
        print(f"Malicious IPs appended to {malicious_ip_file}")
    else:
        print("No malicious IPs detected with more than 100 malicious packets.")

    # Step 4.3: Log each malicious packet to another CSV file
    malicious_packet_file = "logs/malicious_packets.csv"
    malicious_packets = data[data['Malicious'] == 1][required_columns]
    malicious_packets.to_csv(malicious_packet_file, mode='a', index=False, header=not os.path.exists(malicious_packet_file))
    print(f"Logged all malicious packets to {malicious_packet_file}")
...
if __name__ == "__main__":
    ...
    csv_file = 'network_capture/captured_traffic.pcap_flows.csv'
